...
% https://github.com/joramvd/eegpreproc/blob/b7082e1fdf43faa75df94d2b7eb19ec5205e5af2/eegpreproc.m
...
for cEvents = 1:numel(EEG.epoch)
    ...
    %event = EEG.epoch(cEvents).eventtype(index); orignal script version
    event = EEG.event(cEvents).type(index); 
    if iscell(event)
        event = event{1};
    end
    if ischar(event)
        %event = string2double(event);orignal script version
